...
public partial class SpentInAccountReportViewModel : BaseReportPageViewModel<SpentInAccountReportItemViewModel>
{
    public SpentInAccountReportViewModel(IReportsService reportsService, INavigationService navigationService, ITransactionsRequestService transactionsRequestService) : base(reportsService, navigationService, transactionsRequestService)
    {
    }
    ...
    public override async Task LoadReportAsync(FilteredTransactionsRequest searchCriteria)
    {
        ...
        var request = new BaseReportRequest
        {
            ...
            SearchTerm = searchCriteria.SearchTerm
                ...
                await foreach (var reportItem in _reportsService.GetAccountBreakdownReportAsync(request))
        {
            var newItem = new SpentInAccountReportItemViewModel
            {
                TotalIn = reportItem.TotalIn,
                TotalOut = reportItem.TotalOut,
                TotalTransactions = reportItem.TotalTransactions,
                Account  = reportItem.AccountName
            };

            foreach (var yearlyItem in reportItem.YearlyBreakdown)
            {
                var newYearlyItem = new SpentInAccountReportYearlyBreakdownViewModel()
                {
                ...
                {
                    var newMonthlyItem = new SpentInAccountReportMonthlyBreakdownViewModel()
                    {
                    ...
                    {
                        var newDailyItem = new SpentInAccountDailyBreakdownViewModel()
                        {
                            Day = dailyItem.Day,
